...
async function fetchCities() {
  const url = 'https://geo.api.gouv.fr/departements/69/communes?fields=nom,code,population,codesPostaux,centre';
  
  console.log('Fetching Rhône (69) communes from Geo API...');
  try {
    ...
    // Process and filter communes in Rhône (69) with population >= 2000
    const processed = data
      .map(c => {
        let lat = 45.7640; // Lyon lat
        let lng = 4.8357;  // Lyon lng
